...
from src.features.services.schemas import CreateService, ServiceResponse, UpdateService
...
@category_router.get("", response_model=ServerRespModel[List[ServiceResponse]])
...
@category_router.get("/{category_uid}", response_model=ServerRespModel[ServiceResponse])
...
async def add_category(category: CreateService, session: AsyncSession = Depends(get_session)):
    return await category_controller.create_category(category, session)


@category_router.patch("/{category_uid}")
async def update_category(service_uid: uuid.UUID, data: UpdateService, session: AsyncSession = Depends(get_session)):
