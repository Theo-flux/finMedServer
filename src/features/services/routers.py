...
@service_router.get("", response_model=ServerRespModel[List[ServiceResponse]])
async def get_all_roles(session: AsyncSession = Depends(get_session)):
    return await role_controller.get_all_services(session)


@service_router.get("/{service_uid}", response_model=ServerRespModel[ServiceResponse])
...
@service_router.post("")
async def add_role(role: CreateService, session: AsyncSession = Depends(get_session)):
    return await role_controller.create_service(role, session)


@service_router.patch("/{service_uid}")
async def update_role(service_uid: uuid.UUID, data: UpdateService, session: AsyncSession = Depends(get_session)):
